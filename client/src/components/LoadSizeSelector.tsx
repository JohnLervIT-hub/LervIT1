...
import boxesImage from "@assets/generated_images/3d_boxes_floating_isolated.png";
import mediumImage from "@assets/generated_images/3d_chair_table_floating.png";
import largeImage from "@assets/generated_images/3d_sofa_floating_isolated.png";
import apartmentImage from "@assets/generated_images/3d_apartment_model_floating.png";
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {loadSizes.map((size) => {
        ...
        return (
          <Card
            key={size.id}
            className={`p-5 cursor-pointer transition-all duration-200 hover:scale-[1.02] hover:shadow-lg border-2 ${
              isSelected
          ...
          >
            <div className="flex flex-col gap-3">
              <div className="flex items-start justify-between gap-2">
                <div className={`w-16 h-16 rounded-lg overflow-hidden flex items-center justify-center flex-shrink-0 ${isSelected ? 'ring-2 ring-orange-500 ring-offset-2' : ''}`}>
                  <img
                    src={size.image}
                    alt={size.label}
                ...
                {isAiRecommended && (
                  <Badge className="text-xs bg-green-500 text-white shrink-0">AI Suggested</Badge>
                )}
              </div>

              <div>
                <div
                  className={`font-bold text-lg mb-1 ${isSelected ? 'text-orange-600 dark:text-orange-400' : ''}`}
                  data-testid={`text-load-label-${size.id}`}
                >
                  {size.label}
                </div>
