...
import { PhoneCall, PhoneOff, Mic, MicOff, Pause, Play, Settings2, Radio, AlertTriangle, UserRound, X, ArrowRightLeft, Bell, Check } from "lucide-react";
...
export default function AdminVoiceWidget() {
  ...
  const inboundRinging = !!activeCall && callStatus === "ringing" && activeCall.direction !== "outbound";
  ...
  return <div className={`fixed bottom-6 right-6 z-[9999] w-[calc(100vw-2rem)] max-w-sm overflow-hidden rounded-2xl border shadow-2xl voice-shell text-slate-100 transition-all ${inboundRinging ? "ring-4 ring-rose-400/70 animate-pulse" : active ? "ring-2 ring-teal-300/40" : ""}`}>
    <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button onClick={() => setShowRingtonePicker(s => !s)} aria-label="Ringtone settings" aria-expanded={showRingtonePicker} className={`text-slate-300 hover:text-white ${showRingtonePicker ? "text-white" : ""}`} data-testid="voice-widget-ringtone-toggle"><Bell className="h-4 w-4" /></button>
    ...
    {showRingtonePicker && (
      <div className="border-b border-white/10 bg-slate-900/40 px-3 py-2" data-testid="voice-widget-ringtone-picker">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[11px] font-semibold uppercase tracking-wide text-slate-300">Ringtone</span>
          <button onClick={() => setShowRingtonePicker(false)} aria-label="Close ringtone settings" className="text-slate-400 hover:text-white"><X className="h-3.5 w-3.5" /></button>
        </div>
        <ul className="space-y-1">
          {v.ringtones.map(opt => {
            const selected = v.ringtone === opt.key;
            return (
              <li key={opt.key} className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => v.setRingtone(opt.key)}
                  aria-pressed={selected}
                  className={`flex-1 flex items-center gap-2 text-left text-xs px-2 py-1.5 rounded hover:bg-white/10 ${selected ? "bg-white/10" : ""}`}
                  data-testid={`voice-widget-ringtone-option-${opt.key}`}
                >
                  {selected ? <Check className="h-3.5 w-3.5 text-teal-300" /> : <span className="h-3.5 w-3.5 inline-block" />}
                  <span>{opt.label}</span>
                </button>
                <button
                  type="button"
                  onClick={() => v.previewRingtone(opt.key)}
                  className="text-[11px] px-2 py-1 rounded bg-white/10 hover:bg-white/20 text-slate-100"
                  data-testid={`voice-widget-ringtone-preview-${opt.key}`}
                >
                  Preview
                </button>
              </li>
