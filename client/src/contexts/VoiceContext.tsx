...
type VoiceContextValue = { config: VoiceConfig; status: VoiceStatus; call: VoiceCall | null; isReady: boolean; isMuted: boolean; isHeld: boolean; elapsed: number; error: string | null; transferPending: boolean; admins: Admin[]; missedUnread: number;
  ...
  dialpadNumber: string; setDialpadNumber: (value: string) => void;
...
export function VoiceProvider({ children }: { children: ReactNode }) {
  ...
  const [status, setStatusState] = useState<VoiceStatus>("unavailable"); const [call, setCall] = useState<VoiceCall | null>(null); const [isReady, setReady] = useState(false); const [isMuted, setMuted] = useState(false); const [isHeld, setHeld] = useState(false); const [elapsed, setElapsed] = useState(0); const [error, setError] = useState<string | null>(null); const [transferPending, setTransferPending] = useState(false); const [missedUnread, setMissedUnread] = useState(0); const [dialpadNumber, setDialpadNumber] = useState("");
  ...
  const value = useMemo(() => ({ config, status, call, isReady, isMuted, isHeld, elapsed, error, transferPending, admins, missedUnread, ringtone, ringtones: RINGTONE_OPTIONS, dialpadNumber, setDialpadNumber, setStatus, dial, command, transfer, clearCall, requestDevices, resetMissed, setRingtone, previewRingtone }), [config, status, call, isReady, isMuted, isHeld, elapsed, error, transferPending, admins, missedUnread, ringtone, dialpadNumber, setStatus, dial, command, transfer, clearCall, requestDevices, resetMissed, setRingtone, previewRingtone]);
