...
*   **AI-Powered Features:**
    ...
    *   **AI Multi-Item Detection from Photo (Enhanced):** Uses OpenAI Vision API with advanced prompts to detect and catalog EVERY visible item in room photos. Provides itemized inventory with per-item details (name, category, weight, volume, special care requirements, quantity). Auto-aggregates total cubic feet across all detected items for accurate load size classification. Displays comprehensive item list with visual icons, weight/volume breakdowns, and total calculations. Mock fallback simulates multi-item detection for living rooms and bedrooms. Supports both single-item and multi-item photos with intelligent volume-based categorization.
